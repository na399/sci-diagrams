...
/** Existing pipeline, with opt-in measured composition before routing. */
async function run(request: RunRequest): Promise<RunResult> {
  ...
  const intrinsics = measureIntrinsics(mounted); const resolvedSizes = resolveTextSizedElements(entities, mounted, intrinsics);
  ...
  let layout = routeScene(activeEntities, connections, resolvedSizes, provisional.externalText);
  ...
  for (let pass = 0; pass < 2; pass += 1) {
    if (!constrainConnectionLabels(connections, mounted, layout)) { break; }
    measures = measureScene(mounted, intrinsics); finalExternalText = externalTextOf(measures); layout = routeScene(activeEntities, connections, resolvedSizes, finalExternalText);
  }
...
function containmentZIndex(elements: ResolvedElement[]): Map<string, number> {
  ...
  const nestedIds = new Set(elements.map((element) => element.containerId).filter((id): id is string => typeof id === 'string'));
  const depthOf = (element: ResolvedElement): number => {
    let depth = 0; let current = typeof element.containerId === 'string' ? element.containerId : undefined;
    while (current !== undefined && depth <= elements.length) { depth += 1; const parent = byId.get(current)?.containerId; current = typeof parent === 'string' ? parent : undefined; }
    return depth;
  };
...
function attachContainerContent(elements: ResolvedElement[], measures: ElementMeasure[], layout: SceneLayout): void {
  ...
  for (const element of elements) {
    const box = layout.boxes[element.id]; if (typeof element.containerId !== 'string' || !box) { continue; }
    const current = contentById.get(element.containerId); if (!current) { contentById.set(element.containerId, { ...box }); continue; }
    const right = Math.max(current.x + current.width, box.x + box.width); const bottom = Math.max(current.y + current.height, box.y + box.height);
    current.x = Math.min(current.x, box.x); current.y = Math.min(current.y, box.y); current.width = right - current.x; current.height = bottom - current.y;
  }
...
function serialize(): SerializedScene {
  const scene = document.getElementById('eraser-scene'); if (!scene) { throw new Error('__eraser.serialize before __eraser.run'); }
  const css = [FONTS_STYLE_ID, 'eraser-styles'].map((id) => document.getElementById(id)?.textContent ?? '').join(''); return { scene: scene.outerHTML, css };
}
