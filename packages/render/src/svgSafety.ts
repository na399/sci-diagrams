...
export function sanitizeSvgDocument(request: { svg: string; options?: SvgSafetyOptions }): SvgSafetyResult {
  ...
  const parsed = new DOMParser().parseFromString(request.svg, 'image/svg+xml'); const root = parsed.documentElement; const ns = 'http://www.w3.org/2000/svg';
  ...
  const nodes = [root, ...Array.from(root.querySelectorAll('*'))]; if (nodes.length > 20000) { return fail('SVG contains more than 20000 elements.'); }
  ...
  const attributes = new Set(('id class style type xmlns xmlns:xlink href xlink:href xml:space role aria-label aria-labelledby aria-describedby width height viewBox preserveAspectRatio x y x1 y1 x2 y2 cx cy r rx ry d points dx dy rotate transform transform-origin textLength lengthAdjust pathLength version overflow refX refY orient markerWidth markerHeight markerUnits maskUnits maskContentUnits clipPathUnits gradientUnits gradientTransform spreadMethod offset fx fy fr patternUnits patternContentUnits patternTransform').split(' '));
  const ids = new Map<string, Element>(); const index = new Map(nodes.map((node, i) => [node, i])); const location = (node: Element): string => `/svg/elements/${index.get(node) ?? 0}`; const refs = new Map<Element, string[]>();
  const reference = (value: string, node: Element): void => {
    ...
    const matches = [...value.matchAll(/url\(\s*["']?([^"')\s]+)["']?\s*\)/gi)]; if (/url\s*\(/i.test(value) && !matches.length) { problem('Malformed SVG URL.', location(node)); }
    for (const match of matches) { if (!/^#[A-Za-z_][A-Za-z0-9_.:-]*$/.test(match[1]!)) { problem('Only local SVG resource references are allowed.', location(node)); } else { const list = refs.get(node) ?? []; list.push(match[1]!.slice(1)); refs.set(node, list); } }
  };
  const removed = new Set<Element>();
  for (const node of nodes) {
    let depth = 0; for (let parent = node.parentElement; parent; parent = parent.parentElement) { depth += 1; } if (depth > 128) { return fail('SVG nesting exceeds 128 levels.'); }
    ...
    for (const attribute of Array.from(node.attributes)) {
      ...
      if (name.startsWith('data-')) { if (!(options.preserveIdentity && ['data-mdp-id', 'data-mdp-tag'].includes(name))) { node.removeAttribute(name); } continue; }
      ...
      if (name === 'href' || name === 'xlink:href') { if (!/^#[A-Za-z_][A-Za-z0-9_.:-]*$/.test(value)) { problem('External or malformed href is forbidden.', location(node)); } else { const list = refs.get(node) ?? []; list.push(value.slice(1)); refs.set(node, list); } }
      else if (name !== 'style') { reference(value, node); }
      if (['d','points','transform','gradientTransform','patternTransform'].includes(name)) { const numbers = value.match(/[-+]?(?:\d*\.\d+|\d+\.?\d*)(?:[eE][-+]?\d+)?/g) ?? []; if (numbers.some((n) => !Number.isFinite(Number(n)) || Math.abs(Number(n)) > 1e7)) { problem('SVG geometry exceeds the supported numeric range.', location(node)); } }
    }
  }
  if (removed.size) { for (const node of removed) { if (node.localName === 'metadata') { node.remove(); } } warnings.push({ code: 'W_SVG_METADATA', message: 'Nonvisual metadata was removed from the portable vector artifact.', path: '/svg/metadata' }); }
  ...
  const declarations = (source: string, node: Element): Declaration[] => {
    ...
    for (const part of source.split(';')) {
      if (!part.trim()) { continue; } const split = part.indexOf(':'); if (split < 1) { problem('Invalid CSS declaration.', location(node)); continue; }
      const property = part.slice(0, split).trim().toLowerCase(); let value = part.slice(split + 1).trim(); const important = /\s*!important\s*$/i.test(value); value = value.replace(/\s*!important\s*$/i, '').trim();
      if ((!paint.has(property) && property !== 'font') || !value) { problem(`Unsupported CSS property: ${property}`, location(node)); continue; }
      reference(value, node); scratch.cssText = ''; scratch.setProperty(property, value); if (!scratch.getPropertyValue(property)) { problem(`Invalid CSS value for ${property}.`, location(node)); continue; }
      if (property === 'font') { for (const name of ['font-family','font-size','font-weight','font-style','font-variant','font-stretch','line-height']) { const v = scratch.getPropertyValue(name); if (v) { result.push({ property: name, value: v, important }); } } }
      else { result.push({ property, value: scratch.getPropertyValue(property), important }); }
    }
    return result;
  };
  const styleNodes = nodes.filter((node) => node.localName === 'style' && !removed.has(node)); const rules: { selector: string; specificity: number; values: Declaration[]; order: number }[] = [];
  for (const node of styleNodes) {
    const css = node.textContent ?? ''; if (/[\\@]|\/\*/.test(css)) { problem('Only static SVG paint rules are allowed.', location(node)); continue; }
  ...
  if (rules.length * nodes.length > 2000000) { problem('SVG stylesheet expansion exceeds its budget.'); } if (errors.length) { return { ok: false, errors, warnings }; }
  ...
  for (const node of active) {
    ...
    const choose = (values: Declaration[], specificity: number, order: number): void => { for (const declaration of values) { const rank = specificity + (declaration.important ? 1000000 : 0); const old = winners.get(declaration.property); if (!old || rank > old.rank || (rank === old.rank && order >= old.order)) { winners.set(declaration.property, { rank, order, value: declaration.value }); } } };
  ...
  styleNodes.forEach((node) => node.remove()); const activeSet = new Set(active); const adjacency = new Map<Element, Element[]>();
  for (const node of active) { const edges = Array.from(node.children).filter((child) => activeSet.has(child)); for (const id of refs.get(node) ?? []) { const target = ids.get(id); if (!target || !activeSet.has(target)) { problem(`Unresolved SVG reference: #${id}`, location(node)); } else { edges.push(target); } } adjacency.set(node, edges); }
  const visiting = new Set<Element>(); const costs = new Map<Element, number>();
  const expansion = (node: Element, depth = 0): number => { if (visiting.has(node) || depth > 256) { throw new Error('Cyclic or excessively deep SVG resources.'); } const known = costs.get(node); if (known !== undefined) { return known; } visiting.add(node); let cost = 1; for (const next of adjacency.get(node) ?? []) { cost += expansion(next, depth + 1); if (cost > 200000) { throw new Error('SVG resource expansion exceeds its budget.'); } } visiting.delete(node); costs.set(node, cost); return cost; };
  ...
  if (options.namespace) {
    ...
    for (const node of active) {
      ...
      for (const attribute of Array.from(node.attributes)) {
        ...
        if (attribute.namespaceURI) { node.setAttributeNS(attribute.namespaceURI, attribute.name, value); } else { node.setAttribute(attribute.name, value); }
      }
    }
  }
  const comments = parsed.createTreeWalker(root, NodeFilter.SHOW_COMMENT); const toRemove: Node[] = []; while (comments.nextNode()) { toRemove.push(comments.currentNode); } toRemove.forEach((node) => node.parentNode?.removeChild(node));
