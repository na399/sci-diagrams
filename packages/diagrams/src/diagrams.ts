...
import { buildHtmlDocument, serializeSvgScene, type SvgExportOptions, type SvgIssue, type Box, type SceneLayout, type PublicationOptions, type PublicationReport } from '@eraserlabs/render';
...
function toDiagramJson(authored: readonly AuthoredRecord[], layout: SceneLayout, flowIds: readonly string[] = []): DiagramJson {
  ...
  for (const { id, kind, source } of authored) {
    if (kind === 'entity') { const element = structuredClone(source) as DiagramJsonEntity; const box = layout.boxes[id]; if (box) { element.x = box.x; element.y = box.y; element.width = box.width; element.height = box.height; } if (flow.has(id)) { element.layoutItem = 'flow'; } entities.push(element); continue; }
    ...
    if (geometry && origin) { const [originX, originY] = origin; const label = geometry.labelBox; element.x = originX; element.y = originY; element.points = geometry.points.map(([x, y]) => ({ x: x - originX, y: y - originY })); if (label) { element.labelPlacement = { x: Math.round(label.x - originX), y: Math.round(label.y - originY), width: Math.round(label.width), height: Math.round(label.height) }; } }
...
export async function createRenderer(options: RendererOptions): Promise<Renderer> {
  ...
  async function preparePage(): Promise<Page> { const page = await context.newPage(); await page.addInitScript({ path: iifePath }); await page.goto('data:text/html,<!doctype html><html><head></head><body></body></html>'); await page.evaluate((setup) => window.__eraser.setup(setup), pageSetup); if (fontsRequest) { await injectFonts(page, fontsRequest); } return page; }
  ...
  return {
    ...
    async lintSvg(svg: string, lintOptions: PublicationOptions = {}): Promise<PublicationReport> { const page = await acquire(); try { return await page.evaluate((request) => window.__eraser.lintSvg(request), { svg, options: lintOptions }); } finally { release(page); } },
    render: (async (request: RenderRequest) => {
      const requested: OutputRequest = request?.outputs ?? defaultOutputs; const tracker = new TimeTracker(); const resolved = await resolver.resolve(request); tracker.mark('resolve'); tracker.merge('resolve', resolved.meta.timingsMs);
